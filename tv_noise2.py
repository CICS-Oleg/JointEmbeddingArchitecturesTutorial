...
class LinearARn(nn.Module):
    ...
    def forward(self, x_lags):
        """
        x_lags: Tensor of shape [B, n_lags, Dx]
        """
        B = x_lags.shape[0]
...
def run_sigma(
    ...
    def predvae_loss(m):
        ...
        recon_next = F.mse_loss(xhat_next, x_n_tr)

        kl = diag_gaussian_kl(mu, logvar)

        return recon_t + lambda_pred * recon_next + beta_kl * kl
    ...
    with torch.no_grad():
        _, _, _, _, ztr, zhat_p_tr = predvae(x_t_tr)
        _, _, _, _, zte, zhat_p_te = predvae(x_t_te)
    ...
    # training pairs: (x_{t-1}, x_t) -> x_{t+1}
    x_tm1_tr = x_tr[:-2]
    x_t_tr2 = x_tr[1:-1]
    x_n_tr2 = x_tr[2:]
    ...
    r2_ar2 = r2_score(
        s_n_te2.cpu().numpy(),
        ols_fit_predict(ztr_2.cpu().numpy(), s_n_tr2.cpu().numpy(), zte_2.cpu().numpy())
    )
    ...
    x_t_tr_s = x_tr[:-seasonal_lag]
    x_n_tr_s = x_tr[seasonal_lag:]

    s_n_tr_s = s_tr[seasonal_lag:]

    x_t_te_s = x_te[:-seasonal_lag]
    x_n_te_s = x_te[seasonal_lag:]

    s_n_te_s = s_te[seasonal_lag:]
    ...
    r2_sar = r2_score(
        s_n_te_s.cpu().numpy(),
        ols_fit_predict(ztr_s.cpu().numpy(), s_n_tr_s.cpu().numpy(), zte_s.cpu().numpy())
    )

    # --- AR(n) ---
    ar_n_lags = 4
    arn = LinearARn(Dx=Dx, Dz=Dz, n_lags=ar_n_lags).to(device)

    x_lags_tr, x_n_tr_n = make_lagged_tensor(x_tr, ar_n_lags)
    x_lags_te, x_n_te_n = make_lagged_tensor(x_te, ar_n_lags)

    s_n_tr_n = s_tr[ar_n_lags:]
    s_n_te_n = s_te[ar_n_lags:]
    ...
    if make_timeseries:
        ...
        # PredVAE
        predvae_yhat = ols_fit_predict(
            zhat_p_tr.cpu().numpy(),
            s_n_tr.cpu().numpy(),
            zhat_p_te.cpu().numpy()
        )

        preds["PredVAE"] = predvae_yhat

        # --- AR(2) time-series ---
        ar2_yhat = ols_fit_predict(
            ztr_2.cpu().numpy(),
            s_n_tr2.cpu().numpy(),
            zte_2.cpu().numpy()
        )
        ...
        # --- Seasonal AR time-series ---
        sar_yhat = ols_fit_predict(
            ztr_s.cpu().numpy(),
            s_n_tr_s.cpu().numpy(),
            zte_s.cpu().numpy()
        )

        preds[f"SAR({seasonal_lag})"] = sar_yhat

        # --- AR(n) time-series ---
        arn_yhat = ols_fit_predict(
            ztr_n.cpu().numpy(),
            s_n_tr_n.cpu().numpy(),
            zte_n.cpu().numpy(),
        )
        preds[f"AR({ar_n_lags})"] = arn_yhat
